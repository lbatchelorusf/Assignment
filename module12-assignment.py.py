...
def analyze_customer_segments():
    ...
    segment_avg_spend = customer_df.groupby("Segment")["Monthly Spend"].mean().sort_values(ascending=False)
...
def forecast_department_sales():
    ...
    dept = pd.DataFrame({
        ...
        "Bakery": 5000+np.linspace(0,1000,24)+mp.random.normal(0,400,24),
    }, index = dates)
    smoothed = dept.rolling(3, min_periods=1).mean()
    smoothed.plot(title="Dept Sales (3-mo rolling Mean)")
    plt.tight_layout(); plt.show()
